...
export default async function LessonDetailPage({ params, searchParams }: { params: { id: string }, searchParams: { occ?: string } }) {
  ...
  const t = {
    ...
    edit: locale === "es" ? "Editar clase" : "Edit lesson",
    cancellations: locale === "es" ? "Solicitudes de cancelación" : "Cancellation requests",
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-3">
      ...
      <div className="text-sm flex items-center gap-2">{t.student}: <img src={lesson.student?.user?.image ?? '/avatar-placeholder.svg'} alt="avatar" className="w-6 h-6 rounded-full border object-cover" /> {lesson.student?.user?.firstName} {lesson.student?.user?.lastName}</div>
      ...
      <div className="mt-4">
        <a className="border px-3 py-2 rounded inline-block" href={`/teacher/lessons/${lesson.id}/edit`}>{t.edit}</a>
      </div>
      {lesson.cancelRequests?.length > 0 && (
        <div className="mt-6">
          <h2 className="font-semibold">{t.cancellations}</h2>
              ...
              <li key={r.id} className="text-sm text-gray-700 flex items-center justify-between">
                ...
                {r.status === 'PENDING' && (
                  <span className="flex gap-2">
                    <button className="border px-2 py-1 rounded" onClick={async () => { await fetch(`/api/lessons/cancel-requests/${r.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'APPROVE' }) }); location.reload(); }}>{t.approve}</button>
                    <button className="border px-2 py-1 rounded" onClick={async () => { await fetch(`/api/lessons/cancel-requests/${r.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'REJECT' }) }); location.reload(); }}>{t.reject}</button>
                  </span>
                )}
              </li>
            ))}
